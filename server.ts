...
- **تحليل المنافسين:** دراسة السوق ومعرفة الكلمات الأكثر ربحية وأقلها تكلفة.
- **تحديد دقيق للجمهور:** استهداف اهتمامات، سلوكيات، وفئات عمرية محددة بدقة بالغة.
- **تصميم إعلانات جذابة:** استخدام صور، فيديوهات، ونصوص تحث العميل على اتخاذ قرار فوري (Direct Response Copywriting).
- **إعادة الاستهداف (Remarketing):** لإقناع من زاروا موقعك من قبل بالشراء مجدداً.
...
- نوفر لك **تقارير دورية** (أسبوعية أو شهرية حسب الاتفاق) تفصيلية وسهلة الفهم.
- تشمل التقارير: عدد النقرات، حجم الظهور، التكلفة الفعلية لكل نقرة، وعدد العملاء أو الاتصالات التي نتجت عن الإعلانات.
- نناقش معك في كل تقرير التوصيات والتحسينات المقترحة للشهر التالي لزيادة الكفاءة وخفض المصاريف غير الضرورية.
...
النتائج لدينا قابلة للقياس والتحليل دائماً!`;
        ...
        if (msg.includes("سعر") || msg.includes("أسعار") || msg.includes("اسعار") || msg.includes("باقة") || msg.includes("باقات") || msg.includes("بكم") || msg.includes("تكلفة") || msg.includes("طرق الدفع") || msg.includes("الدفع")) {
          if (msg.includes("خرائط") || msg.includes("خريطة") || msg.includes("جوجل")) {
            return `باقات تحسين خرائط جوجل 📍 من وكالة **دلّني** مصممة لتناسب مختلف الميزانيات:
1. **الباقة الأساسية (1,500 ج.م / شهرياً):** تشمل إنشاء وتوثيق النشاط التجاري، إضافة البيانات وتعديل الأوقات والخدمات، ورفع 20 صورة محسنة.
2. **الباقة المتقدمة (2,500 ج.م / شهرياً):** الباقة الأكثر طلباً! تشمل الأساسيات + تحسين شامل لـ الكلمات المفتاحية (Local SEO)، إدارة ومتابعة التقييمات، والرد على تعليقات العملاء، مع كتابة 5 مراجعات أسبوعية ممتازة.
3. **الباقة الشاملة (4,000 ج.م / شهرياً):** لحرية تامة وتصدر دائم. تشمل المتقدمة + حملات إعلانية ذكية للخرائط على محرك البحث وتحديثات مستمرة أسبوعياً وتقارير أداء دورية مكثفة.

أي الباقات تجدها مناسبة لحجم نشاطك الحالي؟ 🏪`;
          }
          if (msg.includes("ناشئة") || msg.includes("صغيرة") || msg.includes("مشاريع")) {
            return `نعم، نحن ندعم المشاريع الناشئة والصغيرة بقوة! 🏪
نوفر باقة مرنة ومخفضة مخصصة للمشاريع الجديدة تبدأ من **1,000 ج.م فقط**، تشمل توثيق نشاطك التجاري على الخرائط وضبط الإعدادات الأساسية لضمان إطلاق متميز لمشروعك في منطقتك الجغرافية.

هدفنا هو أن ننمو معاً؛ عندما يكبر مشروعك وتزداد أرباحك، يمكنك الترقية للباقات المتقدمة!`;
          }
          return `مرحباً بك! باقات وخدمات **دلّني** متنوعة ومرنة للغاية ومصممة لمساعدتك على النجاح:
- **باقات الخرائط (GMB):** تبدأ من 1,500 ج.م شهرياً.
- **باقات إدارة الإعلانات الممولة:** يتم تحديد تكلفة الإدارة بناءً على حجم الميزانية المرصودة للمنصات (تبدأ عمولة الإدارة من 1,500 ج.م شهرياً).
- **طرق الدفع المتاحة:** نقبل الدفع عبر فودافون كاش، إنستاباي (InstaPay)، التحويلات البنكية المباشرة، أو نقداً في مقر الوكالة.

للحصول على تسعير مخصص متكامل لمشروعك، يسعدنا جداً حجز استشارة مجانية لك! 💳`;
        }
...
هل يمكنك توضيح استفسارك أكثر؟ أو اختيار أحد المواضيع السريعة بالأسفل (مثل باقات الخرائط، رفع التقييمات، أو حجز استشارة مجانية) لأعطيك إجابة دقيقة وحلولاً عملية فورية! 🚀`;
      ...
      const systemInstruction = `أنت "دلّوب" (Dalloub)، المساعد الذكي التفاعلي والمستشار التسويقي لوكالة "دلّني" (Dalni) للتسويق الرقمي المتكامل في مصر والوطن العربي.
...
- تحدث بلهجة عربية ودية، مهذبة، واحترافية مع مسحة مصرية لطيفة وذكية تناسب العملاء وأصحاب الأعمال.
...
- اذكر مميزات الوكالة مثل: النتائج القابلة للقياس، الفهم العميق للسوق المصري والخليجي، والتواصل المباشر والسهل عبر الواتساب.

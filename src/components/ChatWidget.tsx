...
export default function ChatWidget() {
  ...
  const send = async (raw: string) => {
    ...
    setTeaser(false);
    setError("");
    const history = [...messages.filter((m) => m.text !== WELCOME.text).slice(-10), { role: "user", text } as ChatMessage];
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: text,
          history: history.map((m) => ({ role: m.role, message: m.text })),
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "خطأ في الاتصال");
      const reply: ChatMessage = { role: "model", text: data.text || "عذراً، لم أستطع فهم طلبك حالياً." };
      const final = [...next, reply];
      setMessages(final);
      persist(final);
    } catch {
      setError("حدث خطأ مؤقت — حاول مجدداً أو تواصل معنا مباشرة عبر واتساب.");

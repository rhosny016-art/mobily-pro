...
/**
 * Memoized service card. The render output depends entirely on `service` and
 * `index`, so wrapping it lets the parent Services grid skip re-rendering all
 * cards when (e.g.) only the filters change elsewhere.
 */
function ServiceCardInner({ service, index = 0 }: { service: Service; index?: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -inset-[1px] rounded-[24px] bg-gradient-to-l from-[#0066CC] via-[#F97316] to-[#10B981] opacity-0 group-hover:opacity-70 blur-[3px] transition-all duration-500 pointer-events-none scale-[1.01]" />
      ...
      <div className={`relative h-full flex flex-col bg-white rounded-[24px] border p-5 sm:p-7 transition-all duration-500 overflow-hidden ${borderClass}`}>
        ...
        <div className="absolute -top-12 -left-12 w-36 h-36 rounded-full bg-gradient-to-br from-blue-500/15 via-emerald-500/10 to-amber-500/15 blur-2xl transition-all duration-700 group-hover:scale-175 group-hover:opacity-90 pointer-events-none" />
